...
//   // vec3 color=vec3(1.0)*line(uv.x,uv.y,1.0,1.0);
//   vec3 color=vec3(1.0)*line(v_position.y,sin(v_position.x*3.1415926*2.0)*1.2,0.1,0.01);
...
void main(){
  ...
  vec3 color=line(v_uv.y,0.5,0.01,0.005)*axis_color;
  color+=line(v_uv.x,0.5,0.01,0.005)*axis_color;
  color+=circle(v_uv.xy,vec2(0.5),0.4,0.01)*axis_color;
  color+=circle(v_uv.xy,vec2(0.5),0.3,0.01)*axis_color;
  color+=circle(v_uv.xy,vec2(0.5),0.2,0.01)*axis_color;
  color+=sweep(v_uv.xy,vec2(0.5),0.4,0.001,0.005)*vec3(0.1,0.3,1.0);
  gl_FragColor = vec4(color,1.0);
}
`
// float circle(vec2 pt, vec2 center, float radius,float line_width){
